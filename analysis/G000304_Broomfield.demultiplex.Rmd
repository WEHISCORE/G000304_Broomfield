...
We might consider decreasing the acceptance probability, e.g., to `pAcpt` $= 0.9 ^ {n_{HTO pools}}$ = `r round(0.9^length(unique(valid_hto_combos_df$Pool)), 2)`.
However, decreasing the acceptance probability will decrease the number of `uncertain` or `negative`-assigned droplets, in this case from `r scales::number(sum(demuxmix_df$Prob < 0.9^nrow(hto)), big.mark = ",")` to `r scales::number(sum(demuxmix_df$Prob < 0.9^length(unique(valid_hto_combos_df$Pool))), big.mark = ",")`.
...
- [ ] Unknown HTO combination is the 3rd most frequent (HTOs 1 & 15); see EDA_HTO.R
